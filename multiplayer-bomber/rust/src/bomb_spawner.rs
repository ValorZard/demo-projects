...
use godot::meta::conv::{ByValue, ByVariant};
use godot::prelude::*;
...
impl ToGodot for BombArgs {
    ...
    fn to_godot(&self) -> Self::Via {
        varray![self.position, self.player_id]
    }
}
...
impl IMultiplayerSpawner for BombSpawner {
    fn ready(&mut self) {
        let callable = Callable::from_object_method(&self.to_gd(), "spawn_bomb");
        self.base_mut().set_spawn_function(&callable);
    }
}
