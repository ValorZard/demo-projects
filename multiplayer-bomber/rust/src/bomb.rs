...
use godot::classes::{Area2D, IArea2D, PhysicsRayQueryParameters2D, TileMap};
use godot::prelude::*;
...
pub struct Bomb {
    #[var]
    pub from_player: i64,
    is_area: Vec<Gd<Node2D>>,
    base: Base<Area2D>,
}

#[godot_api]
impl IArea2D for Bomb {
    fn ready(&mut self) {
        self.base().signals().body_entered().connect_other(self, Bomb::body_entered);
        self.base().signals().body_exited().connect_other(self, Bomb::body_exited);
    }
}
...
impl Bomb {
    ...
    fn explode(&self) {
        ...
        for mut collider in self.is_area.clone() {
    ...
    fn body_entered(&mut self, body: Gd<Node2D>) {
        self.is_area.push(body);
    }

    #[func]
    fn body_exited(&mut self, body: Gd<Node2D>) {
        self.is_area.retain(|b| b != &body);
    }
}